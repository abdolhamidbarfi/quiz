...
interface RadioCardInterface extends React.DetailedHTMLProps<React.InputHTMLAttributes<HTMLInputElement>, HTMLInputElement> {
    title: string
    name: string
    index: number
...
const RadioCard: React.FC<RadioCardInterface> = ({ title, name, index, className , ...params}) => {
    return (
        <div className={`flex items-center ${className}`}>
            <label className="relative flex gap-2 items-center p-2 rounded-full cursor-pointer" htmlFor={`${index}${name}`}>
                <input name={name as string} type="radio" {...params}
                ...
                <span className="absolute text-indigo-100 transition-opacity opacity-0 pointer-events-none top-2/4 left-2/4 -translate-y-2/4 -translate-x-2/4 peer-checked:opacity-100">
                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-3.5 w-3.5 ${className === "text-green-400" ? "text-green-400 " : ""}`} viewBox="0 0 16 16" fill="currentColor">
